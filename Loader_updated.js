  ...
  if (obj.system) {
    ...
    metadata.forEach((mtd, key) => {
      if (mtd.table_name) { //if it's a table
        ...
        sensor_tables = ['Camera', 'CCDSensor', 'AnotherCameraSensor', 'Example', 'Occupancy', 'TempSensor', 'WiFiAP'];
       // sensor_table_integration_ingestor = any(tableName in table_name for loaderIngestor in sensor_tables)
        //sensor_table_integration_ingestor = list(filter(lambda x: table_name in x, sensor_tables))
